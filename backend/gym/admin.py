...
from .models import Tenant, WhatsAppSession, SubscriptionPlan, Member, Payment, NotificationLog


@admin.register(Tenant)
class TenantAdmin(admin.ModelAdmin):
    list_display = ('name', 'owner', 'plan_saas', 'is_active', 'created_at')
...
class MemberAdmin(admin.ModelAdmin):
    list_display = ('last_name', 'first_name', 'tenant', 'current_plan', 'due_date', 'status', 'phone')
...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ('id', 'member', 'amount', 'provider', 'status', 'created_at', 'paid_at')
